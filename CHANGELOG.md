...
- **Standards-Compliant El Torito Bootloader with ISOLINUX & Syslinux Boot-Info-Table (`HBCD/ISOLINUX.BIN`)**: Configured `HBCD/ISOLINUX.BIN` as the El Torito No-Emulation bootloader (2048 bytes / 4 sectors) with dynamic 56-byte boot-info-table patching (PVD LBA 16, Boot file LBA, file size, and 32-bit checksum). This enables SeaBIOS to cleanly load and execute ISOLINUX, which parses `HBCD/ISOLINUX.CFG` and chainloads `HBCD/BOOT/CHAIN.C32 ntldr=/HBCD/XP/XP.BIN` into RAM to boot Mini Windows XP without BIOS CPU freeze.
...
- **Node.js & Python ISO Packaging Engines (`tools/build_minixp_iso.js` / `tools/build_minixp_iso.py`)**: Developed a standalone zero-dependency Node.js ISO-9660 + Joliet Level 3 + El Torito builder alongside the Python script, generating compact 61.8 MB bootable images (`minixp.iso`) with automatic boot-info-table patching and checksum computation.
