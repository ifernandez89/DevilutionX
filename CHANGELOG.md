...
- **In-Browser x86 Cycle-Accurate Virtual Machine (`Packaging/minixp-wasm/`)**: Added a dedicated, isolated x86 virtual machine powered by `v86` compiled to WebAssembly (WASM). Emulates full Pentium II / i686 instruction sets, protected mode paging, 512 MB emulated RAM (providing ample headroom for decompressed 46.2 MB WIM RAMDisk + Windows XP kernel + Desktop), Cirrus/VESA VBE standard graphical framebuffer, ATAPI IDE CD-ROM controller, PIT/PIC/RTC timers, and PS/2 mouse pointer lock capture (`ESC` to release).
