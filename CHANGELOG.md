...
- **Direct NTLDR Real-Mode El Torito Bootloader & RAMDisk Configuration (`tools/generate_boot_sector.js` / `MiniXp/HBCD/XP/X`)**: Replaced ISOLINUX/Syslinux with a custom 16-bit real-mode El Torito bootloader (`minixp_boot.bin`). Fixed RAMDisk image length allocation in `MiniXp/HBCD/XP/X` (`/rdimagelength=48517170`) to stream the entire 46.2 MB `XP.wim` image into RAM. When SeaBIOS boots, the sector displays `Booting Mini Windows XP...`, streams `XP.BIN` (370 KB NTLDR) directly into RAM (`0x2000:0000`) across 64 KB segment boundaries, and transfers execution to NTLDR with `DL` set to the active boot drive. This bypasses all Syslinux filesystem parsing hangs (`ETCD`), booting Mini Windows XP directly.
...
- **Standards-Compliant `pycdlib` ISO Engine with Direct NTLDR Real-Mode Bootloader (`tools/build_iso_pycdlib.py` / `tools/generate_boot_sector.js`)**: Integrated `pycdlib` to construct 100% specification-compliant ISO9660 + Joliet Level 3 + El Torito boot images (`minixp.iso`). The boot sector contains a custom 16-bit real-mode loader (`BOOT.BIN`) that streams `XP.BIN` (370 KB NTLDR) directly into RAM (`0x2000:0000`) using BIOS INT 13h Extensions (AH=42h) and executes Windows XP directly without ISOLINUX or SeaBIOS read errors (`code 0004`).
...
- **Same-Origin GitHub Pages CI/CD Pipeline (`.github/workflows/deploy-pages.yml`)**: Automated the build and deployment pipeline in GitHub Actions to build `minixp.iso` using the direct NTLDR loader Node.js script (`tools/build_minixp_iso.js`) and publish it into `dist/minixp/minixp.iso`. This eliminates legacy `pycdlib`/ISOLINUX builds on GitHub Pages deployments, ensuring deployed web applications run the direct real-mode NTLDR bootloader.
