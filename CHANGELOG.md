...
- **RAMDisk Configuration Fix (`MiniXp/HBCD/XP/X`)**: Corrected `/rdimagelength` from `3161088` (3 MB, causing NTLDR to freeze after loading a truncated WIM) to `48517170` (46.2 MB, the full `XP.wim` image size). Removed stale `/iso=\ISO\Hiren'sBootCD.iso` reference.
