...
- **Direct NTLDR Real-Mode El Torito Bootloader (`tools/generate_boot_sector.js`)**: Replaced ISOLINUX/Syslinux with a custom 16-bit real-mode El Torito bootloader (`minixp_boot.bin`). Fixed segment addressing (`DS = CS`) and added a BIOS drive fallback retry loop (`0x9F` -> `0xE0` -> `0x80`) for INT 13h Extensions (AH=42h). When SeaBIOS boots, the sector displays `Booting Mini Windows XP...`, streams `XP.BIN` (370 KB NTLDR) directly into RAM (`0x2000:0000`) across 64 KB segment boundaries, and transfers execution to NTLDR with `DL` set to the active boot drive. This bypasses all Syslinux filesystem parsing hangs (`ETCD`), booting Mini Windows XP directly.
