/**
 * Quake 1 WASM — Slipgate 3D WebAssembly Controller
 * Powers 60 FPS Quake 1 emulation with TyrQuake, WebAudio, Save States & Gamepads
 */
...
(() => {
    ...
    let activeRomName = 'Quake 1: Episode 1 (Shareware)';
    ...
    const playQuakeBtn = document.getElementById('playQuakeBtn');
    ...
    // Custom PAK Dropzone
    const quakeDropzone = document.getElementById('quakeDropzone');
    const pakFileInput = document.getElementById('pakFileInput');
    const browsePakBtn = document.getElementById('browsePakBtn');
    ...
    function focusGameCanvas() {
        ...
        const canvas = document.getElementById('quake-screen');
        if (canvas) {
            canvas.tabIndex = 0;
            canvas.focus();
        }
    ...
    // Launch Quake with TyrQuake
    async function launchQuake(customRom = null, romLabel = 'Quake 1: Episode 1 (Shareware)') {
        showLoading('Iniciando Quake 1...', 'Cargando motor TyrQuake WebAssembly & PAK0...');
        ...
        try {
            ...
            const retroarchConfig = {
                ...
                input_player1_a: 'space',     // Jump
                input_player1_y: 'e',         // Use
                input_player1_l: 'shift',     // Run
                input_player1_start: 'enter'  // Enter / Menu
            };

            const romTarget = customRom || 'data/id1/pak0.pak';
            ...
            try {
                currentEmulator = await Nostalgist.launch({
                    ...
                    element: canvas,
                    resolveCoreJs() {
                        return 'core/tyrquake_libretro.js';
                    },
                    resolveCoreWasm() {
                        return 'core/tyrquake_libretro.wasm';
                    },
                    retroarchConfig
                });
            } catch (localErr) {
                console.warn('[Quake WASM] Fallback a CDN:', localErr);
                currentEmulator = await Nostalgist.launch({
                    core: 'tyrquake',
                    rom: romTarget,
                    element: canvas,
            ...
            console.error('[Quake WASM] Error crítico:', err);
            hideLoading();
    ...
    function updatePauseBtnUI() {
        ...
        if (isPaused) {
            ...
            pauseResumeBtn.style.background = 'rgba(242, 124, 36, 0.3)';
        } else {
    ...
    // Event Listeners
    if (playQuakeBtn) {
        playQuakeBtn.addEventListener('click', () => {
            launchQuake(null, 'Quake 1: Episode 1 (Shareware)');
        });
    }
    ...
    if (crtToggleBtn) {
        crtToggleBtn.addEventListener('click', () => {
            ...
            crtToggleBtn.style.background = crtFilterEnabled ? 'rgba(242, 124, 36, 0.25)' : '';
    ...
    // Drag and Drop Custom PAK
    if (quakeDropzone) {
        quakeDropzone.addEventListener('dragover', (e) => {
            e.preventDefault();
            quakeDropzone.classList.add('dragover');
        });
        quakeDropzone.addEventListener('dragleave', () => {
            quakeDropzone.classList.remove('dragover');
        });
        quakeDropzone.addEventListener('drop', (e) => {
            e.preventDefault();
            quakeDropzone.classList.remove('dragover');
            if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
                handlePakFile(e.dataTransfer.files[0]);
            }
        });
    }

    if (browsePakBtn && pakFileInput) {
        browsePakBtn.addEventListener('click', () => pakFileInput.click());
        pakFileInput.addEventListener('change', (e) => {
            if (e.target.files && e.target.files.length > 0) {
                handlePakFile(e.target.files[0]);
            }
        });
    }

    function handlePakFile(file) {
        if (!file.name.toLowerCase().endsWith('.pak') && !file.name.toLowerCase().endsWith('.zip')) {
            showToast('Por favor selecciona un archivo .pak o .zip', '⚠️');
            return;
        }
        showToast(`Cargando ${file.name}...`, '📁');
        launchQuake(file, file.name);
    }
    ...
    window.addEventListener('gamepaddisconnected', () => {
        if (gamepadDot) gamepadDot.style.background = '#645f6e';
        if (gamepadText) {
