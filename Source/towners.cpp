...
void TalkToTowner(Player &player, int t)
{
	...
	// FEATURE 3: Complete NPC Facing Polish - Make all town NPCs face the player during interaction
	// Calculate direction from towner to player
	Direction playerDirection = GetDirection(towner.position, player.position.tile);
	
	// Update towner's animation to face the player using existing frames
	if (towner._ttype == TOWN_WITCH) {
		// Adria has 19 frames (0-18), use different frame ranges for different directions
		std::vector<uint8_t> facingAnimOrder;
		switch (playerDirection) {
		case Direction::North:
		case Direction::NorthEast:
		case Direction::NorthWest:
			// Use frames 12-14 for northward facing
			facingAnimOrder = { 12, 13, 14, 13, 12, 13, 14, 13, 12, 13, 14, 13 };
			break;
		case Direction::East:
		case Direction::SouthEast:
			// Use frames 17-18 for eastward facing
			facingAnimOrder = { 17, 18, 17, 18, 17, 18, 17, 18, 17, 18 };
			break;
		case Direction::West:
		case Direction::SouthWest:
			// Use frames 0-2 for westward facing
			facingAnimOrder = { 0, 1, 2, 1, 0, 1, 2, 1, 0, 1, 2, 1 };
			break;
		default: // South
			// Use frames 3-5 for southward facing (default)
			facingAnimOrder = { 3, 4, 5, 4, 3, 4, 5, 4, 3, 4, 5, 4 };
			break;
		}
		...
		towner.animOrder = { TownerAnimOrderStorage.back() };
		towner._tAnimFrameCnt = 0; // Reset animation counter
	}
	else if (towner._ttype == TOWN_STORY) {
		// Cain has 25 frames (0-24), use different frame ranges for different directions
		std::vector<uint8_t> facingAnimOrder;
		switch (playerDirection) {
		case Direction::North:
		case Direction::NorthEast:
		case Direction::NorthWest:
			// Use frames 20-24 for northward facing
			facingAnimOrder = { 20, 21, 22, 23, 24, 23, 22, 21, 20, 21, 22, 23, 24 };
			break;
		case Direction::East:
		case Direction::SouthEast:
			// Use frames 15-19 for eastward facing
			facingAnimOrder = { 15, 16, 17, 18, 19, 18, 17, 16, 15, 16, 17, 18, 19 };
			break;
		case Direction::West:
		case Direction::SouthWest:
			// Use frames 1-5 for westward facing
			facingAnimOrder = { 1, 2, 3, 4, 5, 4, 3, 2, 1, 2, 3, 4, 5 };
			break;
		default: // South
			// Use frames 10-14 for southward facing (default)
			facingAnimOrder = { 10, 11, 12, 13, 14, 13, 12, 11, 10, 11, 12, 13, 14 };
			break;
		}
	...
	else if (towner._ttype == TOWN_SMITH) {
		// Griswold has 16 frames, use different ranges for facing
		std::vector<uint8_t> facingAnimOrder;
		switch (playerDirection) {
		case Direction::North:
		case Direction::NorthEast:
		case Direction::NorthWest:
			// Use frames 12-15 for northward facing
			facingAnimOrder = { 12, 13, 14, 15, 14, 13, 12, 13, 14, 15 };
			break;
		case Direction::East:
		case Direction::SouthEast:
			// Use frames 8-11 for eastward facing
			facingAnimOrder = { 8, 9, 10, 11, 10, 9, 8, 9, 10, 11 };
			break;
		case Direction::West:
		case Direction::SouthWest:
			// Use frames 0-3 for westward facing
			facingAnimOrder = { 0, 1, 2, 3, 2, 1, 0, 1, 2, 3 };
			break;
		default: // South
			// Use frames 4-7 for southward facing (default)
			facingAnimOrder = { 4, 5, 6, 7, 6, 5, 4, 5, 6, 7 };
			break;
		}
	...
	else if (towner._ttype == TOWN_HEALER) {
		// Pepin has 20 frames, use different ranges for facing
		std::vector<uint8_t> facingAnimOrder;
		switch (playerDirection) {
		case Direction::North:
		case Direction::NorthEast:
		case Direction::NorthWest:
			// Use frames 16-19 for northward facing
			facingAnimOrder = { 16, 17, 18, 19, 18, 17, 16, 17, 18, 19 };
			break;
		case Direction::East:
		case Direction::SouthEast:
			// Use frames 12-15 for eastward facing
			facingAnimOrder = { 12, 13, 14, 15, 14, 13, 12, 13, 14, 15 };
			break;
		case Direction::West:
		case Direction::SouthWest:
			// Use frames 4-7 for westward facing
			facingAnimOrder = { 4, 5, 6, 7, 6, 5, 4, 5, 6, 7 };
			break;
		default: // South
			// Use frames 8-11 for southward facing (default)
			facingAnimOrder = { 8, 9, 10, 11, 10, 9, 8, 9, 10, 11 };
			break;
		}
	...
	else if (towner._ttype == TOWN_TAVERN) {
		// Ogden has 16 frames, use different ranges for facing
		std::vector<uint8_t> facingAnimOrder;
		switch (playerDirection) {
		case Direction::North:
		case Direction::NorthEast:
	...
	// For other NPCs (Farnham, Gillian, Wirt), use simpler facing logic with available frames
	else if (!towner.animOrder.empty()) {
		// Generic facing logic for NPCs with existing animation orders
		// Simply adjust the starting frame based on direction to simulate facing
		int frameOffset = 0;
		switch (playerDirection) {
		case Direction::North:
		case Direction::NorthEast:
		case Direction::NorthWest:
			frameOffset = towner._tAnimLen * 3 / 4; // Use last quarter of frames
			break;
		case Direction::East:
		case Direction::SouthEast:
			frameOffset = towner._tAnimLen / 2; // Use middle frames
			break;
		case Direction::West:
		case Direction::SouthWest:
			frameOffset = towner._tAnimLen / 4; // Use first quarter
			break;
		default: // South - use default (no offset)
			frameOffset = 0;
			break;
		}
		
		// Set the animation frame to simulate facing
		towner._tAnimFrameCnt = frameOffset % towner.animOrder.size();
	}
